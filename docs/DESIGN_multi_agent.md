...
| 阶段 | 内容 | 产出 |
|------|------|------|
| **P0** | SQLite 层（`db/`）+ 轻量用户管理 | 建表、repository、用户/项目 REST |
| **P1** | Agent 重构：`agents/` 包、`base`、`registry`、`llm_factory`；CAD 平移 | CAD 能力零回归 |
| **P2** | OrchestratorAgent（规划）+ WorkflowService（执行/事件/落库） | 后端编排闭环 |
| **P3** | 前端三 Tab 重构 + `tabs.js`/`workflow.js`/`scripts.js` + 用户/历史 UI | 完整界面 |
| **P4** | 脚本捕获钩子（FreeCAD）→ Tab3 数据 | 脚本日志可用 |
| **P5** | Mesh / CAE Agent 占位（注册 + 优雅未实现）+ 端到端联调 | 框架可扩展 |

> 本期交付到"框架可用 + CAD 全流程跑通 + Mesh/CAE 占位"，Mesh/CAE 的真实软件接入留待后续。
